...
class InitNet_GNN(nn.Module):
    ...
    def forward(self, img, obj_ids, return_img_feats=False, return_graph_feats=False):
        ...
        for i, block in enumerate(self.pre_query_block):
            graph_feats = block(graph_feats, pre_batch_indices, obj_ids)  # shape: (batch, 64, #keypoints)
        # final query
