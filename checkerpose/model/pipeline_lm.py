...
class StaticGraph_module(nn.Module):
    ...
    def forward(self, x, batch_indices, obj_ids):
        # note: obj_ids start from 1
        out = get_graph_feature(x, self.knn_idx[obj_ids-1], batch_indices)  # shape: (B, 2C, N, K)
...
class Refine_moduleGNN(nn.Module):
    ...
    def forward(self, img_feat, graph_feat, p3d_normed, roi_mask_bit, prev_x_id, prev_y_id, obj_ids):
        ''' Args:
        ...
        obj_ids: batch of obj_ids, shape: (B,), start from 1
        Return:
        ...
        for i, block in enumerate(self.pre_query_block):
            local_feat = block(local_feat, pre_batch_indices, obj_ids)  # shape: (batch, C, #keypoint)
        local_feat = local_feat.permute(0, 2, 1)  # shape: (batch, #keypoint, C)
...
# ablation study for no progressive prediction, need to remove image feature fusion and code prediction
class Refine_moduleGNN_ABwoProg(nn.Module):
    def __init__(self, npoint, p3d_normed, num_filters=256, max_batch_size=64, query_dims=None,
                 local_k=4, leaky_slope=0.01, num_graph_module=2, graph_k=20, graph_leaky_slope=0.2,
                 query_type="mlp", graph_feat_dim=64):
        ''' Args:
        ...
        graph_feat_dim: dim of graph feature from last step (either refine or init)
        '''
        super(Refine_moduleGNN_ABwoProg, self).__init__()
        self.npoint = npoint
        # set the query dims here which will impact the local feature output dim
        if query_type == "mlp":
            self.query_dims = (num_filters, 256, 64) if query_dims is None else query_dims
        ...
        # convert original feature channel to desired one
        self.pre_graph_module = get_MLP_leakyReLU_layers(
            dims=(graph_feat_dim, self.query_dims[0], self.query_dims[0]),
            doLastAct=True, negative_slope=leaky_slope)
    ...
    def forward(self, graph_feat, obj_ids):
        ''' Args:
        graph_feat: shape: (batch, C, #keypoint)
        obj_ids: batch of obj_ids, shape: (B,), start from 1
        Return:
            local_feat -- tensor (B, C, #keypoint) output graph feature
        '''
        batch_size = graph_feat.shape[0]
        # process before graph modules (to convert number of feature channel)
        local_feat = graph_feat.permute(0, 2, 1)  # shape: (batch, #keypoint, C)
        local_feat = self.pre_graph_module(local_feat)  # shape: (batch, #keypoint, C)
        local_feat = local_feat.permute(0, 2, 1)  # shape: (batch, C, #keypoint)
        # get refined graph feature
        pre_batch_indices = self.pre_batch_indices[:batch_size]
        for i, block in enumerate(self.pre_query_block):
            local_feat = block(local_feat, pre_batch_indices, obj_ids)  # shape: (batch, C, #keypoint)
        return local_feat
...
class PoseNet_GNNskip(nn.Module):
    ...
    def forward(self, img, p3d_normed, obj_ids, stage=None):
        ''' Args:
        ...
        output_bits, img_feats, graph_feat = self.init_net(img, obj_ids, return_img_feats=False, return_graph_feats=True)  # shape: (batch, 7, #keypoints), 1 for in/out RoI, 3 for x/y
        ...
        for i in range(num_active_ref):
            ...
            new_bits, graph_feat = self.refine_net[i](img_feat, graph_feat, p3d_normed, roi_mask_bit, pred_x_id, pred_y_id, obj_ids)
...
# ablation study model for PoseNet_GNNskip: remove the progressive prediction part
# still keep the image feature branch for predicting the segmentation features, but no fusion
class PoseNet_GNNskip_ABwoProg(nn.Module):
    def __init__(self, init_net, npoint, p3d_normed, res_log2=6, num_filters=256, max_batch_size=64, query_dims=None,
                 seg_output_dim=2, local_k=4, leaky_slope=0.01, num_graph_module=2, graph_k=20, graph_leaky_slope=0.2,
                 query_type="mlp"):
        ''' Args:
        ...
        leaky_slope: negative slope of the leaky ReLU activation
        '''
        super(PoseNet_GNNskip_ABwoProg, self).__init__()
        self.npoint = npoint
        self.init_net = init_net
        self.res_log2 = res_log2
        self.num_bits = 2 * res_log2 + 1
        ...
        for i in range(self.num_refine_steps):
            ...
            if i == 0:
                ...
                graph_feat_dim_i = query_dims[0]
            block = Refine_moduleGNN_ABwoProg(npoint=npoint, p3d_normed=p3d_normed, num_filters=num_filters,
                                              max_batch_size=max_batch_size, query_dims=query_dims,
                                              local_k=local_k, leaky_slope=leaky_slope, num_graph_module=num_graph_module_i,
                                              graph_k=graph_k, graph_leaky_slope=graph_leaky_slope, query_type=query_type,
                                              graph_feat_dim=graph_feat_dim_i)
        ...
        # final image segmentation
        self.seg_block = nn.Conv2d(num_filters, seg_output_dim, kernel_size=1, padding=0, bias=True)
        # obtain the binary codes from the final graph feature
        if query_type == "mlp":
            self.query_dims = (num_filters, 256, 64) if query_dims is None else query_dims
            self.query_block = MLP_QueryNet(feat_dims=self.query_dims, pt_dim=3, out_dim=self.num_bits, leaky_slope=leaky_slope)
        else:
            raise ValueError("query type {} not supported in Refine_module".format(query_type))
    ...
    def forward(self, img, p3d_normed, obj_ids, stage=None):
        ...
        '''
        num_active_ref = stage if stage is not None else self.num_refine_steps
        # discard the initial localization, only keep the features
        _, img_feats, graph_feat = self.init_net(img, obj_ids, return_img_feats=False, return_graph_feats=True)  # shape: (batch, 7, #keypoints), 1 for in/out RoI, 3 for x/y
        img_feat = img_feats[-1]  # only use the last one
        # refinement step (just refine the features)
        for i in range(num_active_ref):
            if i > 0:
                img_feat = torch.cat([img_feat, img_feats[-i - 1]], dim=1)
            img_feat = self.up_net[i](img_feat)  # preprocess before refinement: upsampling the feature
            graph_feat = self.refine_net[i](graph_feat, obj_ids)
        # final image segmentation step
        output_seg = self.seg_block(img_feat)
        # obtain the codes from the final graph feature
        graph_feat = graph_feat.permute(0, 2, 1)  # shape: (batch, #keypoint, C)
        output_bits = self.query_block(graph_feat, p3d_normed.permute(0, 2, 1))  # shape: (batch, #keypoint, #bits)
        output_bits = output_bits.permute(0, 2, 1)  # shape: (batch, #bits, #keypoint)
        output_roi_bit = output_bits[:, 0:1, :]
        output_x_bits = output_bits[:, 1:(self.res_log2+1), :]
        output_y_bits = output_bits[:, (self.res_log2+1):, :]
        # also convert to the index for inference
        pred_x_id = from_code_prob_to_id(output_x_bits.detach())
        pred_y_id = from_code_prob_to_id(output_y_bits.detach())
        return output_roi_bit, output_x_bits, output_y_bits, output_seg, pred_x_id, pred_y_id
